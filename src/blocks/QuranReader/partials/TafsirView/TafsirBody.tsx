...
import { selectTafsirs, setSelectedTafsirs } from '~/store/slices/QuranReader/tafsirs'
import { BaseResponse, TafsirContentResponse, TafsirsResponse } from '~/types/ApiResponses'
...
const TafsirBody = ({
  ...
  const languageOptions = tafsirSelectionList?.tafsirs
  ...
  const onLanguageSelected = (event) => {
    const newLangValue = event.target.value
    logValueChange('tafsir_locale', selectedLanguage, newLangValue)
    setSelectedLanguage(newLangValue)

    if (tafsirSelectionList) {
      const firstTafsirOfLanguage = getFirstTafsirOfLanguage(tafsirSelectionList, newLangValue)
      if (firstTafsirOfLanguage) {
        const { id, slug } = firstTafsirOfLanguage
        onTafsirSelected(id as number, slug as string)
      }
    }
  }

  const renderTafsir = useCallback(
    (data: BaseResponse): JSX.Element => {
      const tafsirData = data as TafsirContentResponse
      if (!tafsirData || !tafsirData.tafsir) return <TafsirSkeleton />

      const { verses, text, languageId } = tafsirData.tafsir
      ...
      return (
        <div>
          {!text && (
            <TafsirMessage>
              {t('tafsir').translate('tafsir.no-text', tafsirData.tafsir.translatedName.name)}
            </TafsirMessage>
          )}
  ...
  const onVerseNumberChange = (event) => {
    const newVerseNumber = event.target.value
