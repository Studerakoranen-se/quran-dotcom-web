...
import { addLoadedFontFace, selectLoadedFontFaces } from '~/store/slices/QuranReader/font-faces'
...
const useQcfFont = (quranFont: QuranFont, verses: Verse[]) => {
    ...
    if (isFontQCF && verses.length > 0) {
      ...
      getPagesByVerses(verses).forEach((pageNumber) => {
        ...
        const fontFace = new FontFace(
          fontFaceName,
          getQCFFontFaceSource(quranFont, pageNumber, mode as 'light' | 'dark' | 'system'),
        )
  ...
  }, [quranFont, verses, loadedFonts, isFontQCF, currentlyFetchingFonts, onFontLoaded, mode])
}
